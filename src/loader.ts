...
class Loader {
  ...
  static {
    this.#require = Module.createRequire(
      url.pathToFileURL(process.cwd() + '/index').href,
    );
  }
  ...
  private static async importModule(pathToModule: string, useCache = true) {
    const importPath = useCache
      ? pathToModule
      : `${pathToModule}?${Date.now()}`;
  ...
  static async parseJsonFile(pathToModule: string) {
    try {
      ...
      throw new Error(`Error: Could not import JSON module: ${pathToModule}`, {
        cause: error,
      });
    }
  }
  ...
  private static async dynamicImport(pathToModule: string) {
    try {
      ...
      throw new Error(`Error: Could not import module: ${pathToModule}`, {
        cause: error,
      });
    }
  }
