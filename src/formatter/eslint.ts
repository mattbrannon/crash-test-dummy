...
class ESLintFormatter {
  ...
  async formatString(text: string) {
    try {
      const { eslint, linter, config } = await ESLintFormatter.getESLint(
        this.document,
      );
      ...
      const results = linter.verifyAndFix(text, config, {
  ...
  static isLegacyConfigWithParser(
    config: ESLintConfig,
  ): config is LegacyConfigWithParser {
    return (
      !Array.isArray(config) &&
      isDefined((config as LegacyConfigWithParser).parser)
    );
  }

  static getESLintOptions(
    config: ESLintConfig,
    configPath: string,
    workspace: string,
  ): Options {
  ...
  static async getESLint(document: vscode.TextDocument) {
    ...
    const options = ESLintFormatter.getESLintOptions(
      config,
      configPath,
      workspace,
    );
    ...
    Logger.info(
      `${ESLint.configType.toUpperCase()} config loaded from: file://${configPath}`,
    );
    ...
    if (ESLintFormatter.isLegacyConfigWithParser(config)) {
      ...
      linter.defineParser(
        parser,
        Resolver.make(parser).require(document.fileName),
      );
    }
